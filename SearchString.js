...
function loadBook(filename, displayName) {
    ...
    var xhr = new XMLHttpRequest(); //var xhr is made object of class XMLHttpRequest()
    xhr.open("GET", url, true); //Open method has 3 parameter method(get or post),filename,async(true or false)
    xhr.send(); // send the request to server

    xhr.onreadystatechange = function() //check ready state at everystep to find state of response from server
        {
            if (xhr.readyState == 4 && xhr.status == 200) //check server response ,here for ok
            {
                currentBook = xhr.responseText; //give response text to currentBook
